...
from coral.utils.models.load_inr import create_inr_instance, load_inr_model, load_inr_model_sep_lr
from coral.utils.models.get_inr_reconstructions import get_reconstructions
from coral.utils.data.load_modulations import load_dynamics_modulations, load_dynamics_modulations_sep_lr
...
@hydra.main(config_path="config/", config_name="ode_grid.yaml")
def main(cfg: DictConfig) -> None:
    ...
    if load_run_name is not None:
        ...
        grid_channel = tmp["cfg"].inr.depth - 1
        if tmp["cfg"].inr.modulate_scale and tmp["cfg"].inr.modulate_shift:
            grid_channel *= 2
        grid_size = tmp["cfg"].inr.grid_size
        code_dim = latent_dim - grid_channel * grid_size * grid_size
        meta_same_grid_sep_lr_two_stage =tmp["cfg"].optim.meta_same_grid_sep_lr_two_stage
    ...
    if load_run_name is not None:
        inr, alpha, alpha_grid = load_inr_model_sep_lr(
            root_dir / "inr",
        ...
        print(inr)
        modulations = load_dynamics_modulations_sep_lr(
            trainset,
            trainset_extra,
            testset,
            inr,
            ...
            code_dim=code_dim,
            two_stage=meta_same_grid_sep_lr_two_stage,
            batch_size=2,
            data_to_encode=None,
            try_reload=False,
        )
    ...
    if evaluate:
        pred_train_inter_mse, code_train_inter_mse, pred_train_extra_mse, code_train_extra_mse, total_pred_train_mse, detailed_train_eval_mse, code_train_mse_inter_code, code_train_mse_extra_code, code_train_mse_inter_grid, code_train_mse_extra_grid = batch_eval_loop(
                    ...
                    dataset_name, T_train, n_cond, code_dim, visual_first=1, visual_path=os.path.join(run.dir, f'train_{epoch_start}'), visual_mod=grid_size
                )
        ...
        pred_test_inter_mse, code_test_inter_mse, pred_test_extra_mse, code_test_extra_mse, pred_test_mse, detailed_test_mse, code_test_mse_inter_code, code_test_mse_extra_code, code_test_mse_inter_grid, code_test_mse_extra_grid = batch_eval_loop(
                    ...
                    dataset_name, T_train, n_cond, code_dim, visual_first=1, visual_path=os.path.join(run.dir, f'test_{epoch_start}'), visual_mod=grid_size
                )
