...
from coral.utils.models.scheduling import hippo_scheduling
...
from coral.mlp import DerivativeHippo
...
from dynamics_modeling.eval_hippo import batch_eval_loop
...
@hydra.main(config_path="config/", config_name="ode_hippo.yaml")
def main(cfg: DictConfig) -> None:
    ...
    memory_size = cfg.dynamics.memory_size
    ...
    dt = 1
    timestamps_train = torch.arange(0, T_train, dt).float().cuda() + 1
    timestamps_test = torch.arange(0, T_test, dt).float().cuda() + 1
    ...
    model = DerivativeHippo(c, z_train.shape[1], hidden, depth, N=hippo_n, memory_size=memory_size).cuda()
    print(model)
    ...
    for step in range(epochs):
        ...
        for substep, (images, modulations, coords, idx) in enumerate(train_loader):
            ...
            if c0 is None:
                if memory_size  > 0:
                    c0 = torch.zeros_like(modulations[:,0:1,0:1]).repeat(1,memory_size,hippo_n)
                else:
                    c0 = torch.zeros_like(modulations[...,0:1]).repeat(1,1,hippo_n)

            if multichannel:
                modulations = einops.rearrange(modulations, "b l c t -> b (l c) t")

            z_pred = hippo_scheduling(odeint, model, modulations, timestamps_train, c0, epsilon_t)
            loss = ((z_pred - modulations) ** 2).mean()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            code_train_mse += loss.item() * n_samples
            print(substep, loss.item())
        ...
        if T_train != T_test:
            pred_train_inter_mse, code_train_inter_mse, pred_train_extra_mse, code_train_extra_mse, total_pred_train_mse, detailed_train_eval_mse = batch_eval_loop(
                model, inr, c0, train_extra_loader,
        ...
        if True in (step_show, step_show_last):
            ...
            # print(f'{step}: pred train mse {pred_train_mse}, code train mse {code_train_mse}')
