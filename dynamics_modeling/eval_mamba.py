...
from coral.utils.models.scheduling import mamba_scheduling, mamba_scheduling_startT
...
def batch_eval_loop(model, inr, loader, timestamps, detailed_mse, 
    ...
    if interpolation_seq:
        ...
        for images, modulations, coords, idx in loader:
            ...
            with torch.no_grad():
                z_pred = mamba_scheduling(
                    model, modulations, timestamps, ar_mode=True
                )
...
def batch_eval_loop_startT(model, inr, loader, timestamps, detailed_mse, 
                     n, multichannel, z_mean, z_std, dataset_name, 
                     interpolation_seq, startT):
    if interpolation_seq:
        pred_mse_inter = 0
        code_mse_inter = 0
        pred_mse_extra = 0
        ...
        for images, modulations, coords, idx in loader:
            ...
            with torch.no_grad():
                z_pred = mamba_scheduling_startT(
                    model, modulations, timestamps, ar_mode=True, ar_startT=startT
                )
            loss = ((z_pred - modulations) ** 2)
            loss_inter = loss[..., :interpolation_seq].mean()
            loss_extra = loss[..., interpolation_seq:].mean()
