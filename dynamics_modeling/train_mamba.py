...
from coral.utils.models.scheduling import mamba_scheduling
...
from coral.mamba import MyMamba 
...
from dynamics_modeling.eval_mamba import batch_eval_loop, batch_eval_loop_startT
...
@hydra.main(config_path="config/", config_name="mamba.yaml")
def main(cfg: DictConfig) -> None:
    ...
    ar_mode = cfg.dynamics.ar_mode
    d_state = cfg.dynamics.d_state
    ...
    model = MyMamba(input_dim=c*z_train.shape[1], d_model=c*z_train.shape[1], d_state=d_state, n_layer=depth).cuda()
    print(model)
    ...
    if evaluate:
        ...
        Ts = [1, 10, 20, 40]
        for startT in Ts:
            pred_train_inter_mse, code_train_inter_mse, pred_train_extra_mse, code_train_extra_mse, total_pred_train_mse, detailed_train_eval_mse = batch_eval_loop_startT(
                        model, inr, train_extra_loader,
                        timestamps_test, detailed_train_eval_mse,
                        ntrain, multichannel, z_mean, z_std,
                        dataset_name, T_train, startT
                    )

        
            pred_test_inter_mse, code_test_inter_mse, pred_test_extra_mse, code_test_extra_mse, pred_test_mse, detailed_test_mse = batch_eval_loop_startT(
                model, inr, test_loader,
                timestamps_test, detailed_test_mse,
                ntest, multichannel, z_mean, z_std,
                dataset_name, T_train, startT
            )
            print(f'{startT}, train inter mse {pred_train_inter_mse}, train_extra_mse {pred_train_extra_mse}, test inter mse {pred_test_inter_mse}, test extra mse {pred_test_extra_mse}')
        return 
    ...
    for step in range(epochs):
        ...
        for substep, (images, modulations, coords, idx) in enumerate(train_loader):
            ...
            z_pred = mamba_scheduling(model, modulations, timestamps_train, ar_mode=ar_mode)
        ...
        # print(f'{step}: train inter mse {pred_train_inter_mse}, train_extra_mse {pred_train_extra_mse}')
        if True in (step_show, step_show_last):
            if T_train != T_test:
                pred_test_inter_mse, code_test_inter_mse, pred_test_extra_mse, code_test_extra_mse, pred_test_mse, detailed_test_mse = batch_eval_loop(
                    model, inr, test_loader,
                ...
                if multichannel:
                    ...
                    log_dic.update(dic_test_mse)
            wandb.log(log_dic)
            print(f'validate {step}: train inter mse {pred_train_inter_mse}, train_extra_mse {pred_train_extra_mse}, test inter mse {pred_test_inter_mse}, test extra mse {pred_test_extra_mse}')
        else:
            wandb.log(
                {
                    # "pred_train_inter_mse": pred_train_inter_mse,
