...
from coral.mlp import Derivative,SetEncoder
...
from dynamics_modeling.eval_setencoder import batch_eval_loop
...
@hydra.main(config_path="config/", config_name="ode_setencoder.yaml")
def main(cfg: DictConfig) -> None:
    ...
    # T_train = T_train-n_cond
    # T_test = T_test-n_cond
    ...
    model = Derivative(c, 2*z_train.shape[1], hidden, depth).cuda()
    print(model)
    model_cond = SetEncoder(c*z_train.shape[1], n_cond, 1024).cuda()
    print(model_cond)
    ...
    for step in range(epochs):
        ...
        for substep, (images, modulations, coords, idx) in enumerate(train_loader):
            ...
            # if c0 is None:
            #     c0 = torch.zeros_like(modulations[...,0])
            ...
            z_pred = ode_scheduling(odeint, model, augment_states, timestamps_train[n_cond:], epsilon_t)[:,:modulations.shape[1]]
            # import pdb; pdb.set_trace()
        ...
        if T_train != T_test:
            pred_train_inter_mse, code_train_inter_mse, pred_train_extra_mse, code_train_extra_mse, total_pred_train_mse, detailed_train_eval_mse = batch_eval_loop(
                model, model_cond, inr, train_extra_loader,
        ...
        if True in (step_show, step_show_last):
            if T_train != T_test:
                pred_test_inter_mse, code_test_inter_mse, pred_test_extra_mse, code_test_extra_mse, pred_test_mse, detailed_test_mse = batch_eval_loop(
                    model, model_cond, inr, test_loader,
