...
name='eval_dyngrid_ipcl32_total1024_bs32_wf_r3_first40_T64-wonorm2'
id='eval_dyngrid_ipcl32_total1024_bs32_wf_r3_first40_T64-wonorm2'
python3 dynamics_modeling/train_grid.py "inr.grid_channel=$grid_channel" "inr.grid_size=$grid_size" "inr.run_name_suffix=$run_name_suffix" "data.dir=$dir" "data.sub_from=$sub_from" "data.same_grid=$same_grid" "data.dataset_name=$dataset_name" "dynamics.width=$width" "dynamics.depth=$depth" "data.sub_tr=$sub_tr" "data.sub_te=$sub_te" "optim.epochs=$epochs" "data.seq_inter_len=$seq_inter_len" "data.seq_extra_len=$seq_extra_len" "optim.batch_size=$batch_size" "optim.lr=$lr"  "dynamics.teacher_forcing_update=$teacher_forcing_update" "inr.run_name=$run_name" "wandb.name=$name" "wandb.id=$id" "wandb.evaluate=$evaluate"
