...
evaluate=True 
checkpoint_path='/cluster/home1/whh/workspace/wandb/sst-11-22/model/dyn_sst_zone_'$zone_id'_ck.pt'
...
name='dyn_sst_zone_ncond'$n_cond'_evaluate_gt_'$zone_id
id='dyn_sst_S202_zone_bs64_ncond'$n_cond'_evaluate_gt_'$zone_id
python3 dynamics_modeling/train.py  "data.n_cond=$n_cond"  "data.dir=$dir" "data.sub_from=$sub_from" "data.same_grid=$same_grid" "data.dataset_name=$dataset_name" "dynamics.width=$width" "dynamics.depth=$depth" "data.sub_tr=$sub_tr" "data.sub_te=$sub_te" "optim.epochs=$epochs" "data.seq_inter_len=$seq_inter_len" "data.seq_extra_len=$seq_extra_len" "optim.batch_size=$batch_size" "optim.lr=$lr"  "dynamics.teacher_forcing_update=$teacher_forcing_update" "inr.run_name=$run_name" "wandb.name=$name" "wandb.id=$id" "wandb.saved_checkpoint=$saved_checkpoint" "wandb.checkpoint_path=$checkpoint_path" "wandb.evaluate=$evaluate"
