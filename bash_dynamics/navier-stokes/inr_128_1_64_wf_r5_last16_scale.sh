...
inner_steps=3
test_inner_steps=3
name='wf_r5_1_last16_1_64_scale'
update_modulations=False
update_alpha=True
modulate_scale=True 

python3 inr/inr.py "inr.modulate_scale=$modulate_scale" "optim.update_alpha=$update_alpha" "optim.update_modulations=$update_modulations" "optim.inner_steps=$inner_steps" "optim.test_inner_steps=$test_inner_steps" "data.dir=$dir" "data.sub_from=$sub_from" "data.same_grid=$same_grid" "data.dataset_name=$dataset_name" "inr.model_type=$model_type" "data.seq_inter_len=$seq_inter_len" "data.seq_extra_len=$seq_extra_len" "data.sub_tr=$sub_tr" "data.sub_te=$sub_te" "optim.lr_inr=$lr_inr" "inr.depth=$depth" "inr.latent_dim=$latent_dim" "inr.hidden_dim=$hidden_dim" "optim.batch_size=$batch_size" "optim.epochs=$epochs" "wandb.saved_checkpoint=$saved_checkpoint" "wandb.name=$name" # "wandb.checkpoint_path=$checkpoint_path" #"wandb.id=$id" "wandb.dir=$dir" 
