...
evaluate=True 
checkpoint_path='/cluster/home1/whh/workspace/wandb/navier-stokes-nms-40-64-wonorm/model/dyn_wf_r3_last40_T64_bs32-wonorm_ck.pt'
...
run_name='wf_r3_last40_T64-wonorm2' 
name='dyn_wf_r3_last40_T64-wonorm_evalck_on128'
id='dyn_wf_r3_last40_T64-wonorm_evalck_on128'
python3 dynamics_modeling/train_debug.py "data.dir=$dir" "data.sub_from=$sub_from" "data.same_grid=$same_grid" "data.dataset_name=$dataset_name" "dynamics.width=$width" "dynamics.depth=$depth" "data.sub_tr=$sub_tr" "data.sub_te=$sub_te" "optim.epochs=$epochs" "data.seq_inter_len=$seq_inter_len" "data.seq_extra_len=$seq_extra_len" "optim.batch_size=$batch_size" "optim.lr=$lr"  "dynamics.teacher_forcing_update=$teacher_forcing_update" "inr.run_name=$run_name" "wandb.name=$name" "wandb.id=$id" "wandb.saved_checkpoint=$saved_checkpoint" "wandb.checkpoint_path=$checkpoint_path" "wandb.evaluate=$evaluate"
