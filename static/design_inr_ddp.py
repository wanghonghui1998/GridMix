...
def main(cfg: DictConfig) -> None:
    ...
    train_loader = DataLoader(dataset=trainset, batch_size=batch_size, shuffle=(train_sampler is None), sampler=train_sampler)
    ...
    if cfg.distributed:
        inr_with_alpha_in = INR_with_alpha(inr_in, alpha_in)
        inr_with_alpha_out = INR_with_alpha(inr_out, alpha_out)
        inr_with_alpha_in = torch.nn.parallel.DistributedDataParallel(inr_with_alpha_in, device_ids=[cfg.gpu_id])
        inr_with_alpha_out = torch.nn.parallel.DistributedDataParallel(inr_with_alpha_out, device_ids=[cfg.gpu_id])
        inr_in = inr_with_alpha_in.module.inr 
        alpha_in = inr_with_alpha_in.module.alpha 
        inr_out = inr_with_alpha_out.module.inr 
        alpha_out = inr_with_alpha_out.module.alpha 
    ...
    fit_train_mse_in = torch.tensor([0.0]).to(device)
    fit_train_mse_out = torch.tensor([0.0]).to(device)
    
    for step in range(epochs):
        fit_train_mse_in[0] = 0
        fit_test_mse_in = 0
        rel_train_mse_out = 0
        rel_test_mse_out = 0
        fit_train_mse_out[0] = 0
        fit_test_mse_out = 0
        ...
        for substep, (a_s, u_s, za_s, zu_s, coords, idx) in enumerate(
            ...
            inr_with_alpha_in.train()
            inr_with_alpha_out.train()
            ...
            if optim_in:
                ...
                fit_train_mse_in[0] += loss.item() * n_samples
                z0 = outputs["modulations"].detach()

                # mu0 = beta1 * mu0 + (1-beta1)*z0.mean(0)
                # sigma0 = beta1 * sigma0 + (1-beta1)*z0.std(0) + 1e-8

                if cfg.rank == 0 and step_show and substep == 0:
                    u_pred = inr_with_alpha_in(coords, z0)
                    with torch.no_grad():
            ...
            if optim_out:
                ...
                optimizer_out.step()
                loss = outputs["loss"].cpu().detach()
                fit_train_mse_out[0] += loss.item() * n_samples
            ...
            # print(f'{cfg.rank} {substep} {alpha_in.item()} {inr_in.last_layer.linear.weight.mean()}, {alpha_out.item()} {inr_out.last_layer.linear.weight.mean()}')
        if cfg.distributed:
            dist.all_reduce(fit_train_mse_in, op=dist.ReduceOp.SUM, async_op=False)
            dist.all_reduce(fit_train_mse_out, op=dist.ReduceOp.SUM, async_op=False)
        ...
        if cfg.rank == 0:
            ...
            for substep, (a_s, u_s, za_s, zu_s, coords, idx) in enumerate(
                ...
                inr_with_alpha_in.eval()
                inr_with_alpha_out.eval()
                a_s = a_s.cuda()
                u_s = u_s.cuda()
                za_s = za_s.cuda()
                zu_s = zu_s.cuda()
