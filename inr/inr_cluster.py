...
def main(cfg: DictConfig) -> None:
    ...
    modulation_path = '/cluster/home3/whh/workspace/pinn/exp_nvme4a/wandb/navier-stokes-nms-f40-64-wonorm/modulations/wf_r3_first40_T64-wonorm2.pt'
    # modulation_path = '/cluster/home3/whh/workspace/pinn/exp_nvme4a/wandb/navier-stokes-nms-40-64-wonorm/modulations/wf_r3_last40_T64-wonorm2.pt'
    modulation = torch.load(modulation_path, map_location='cpu')
    z_train = modulation['z_train'].permute(0,2,1).reshape(-1, 128)
    z_test = modulation['z_test'].permute(0,2,1).reshape(-1, 128)
    from sklearn.cluster import KMeans
    k=20
    kmeans = KMeans(n_clusters=k, random_state=0, n_init="auto").fit(z_train.numpy())
    labels = torch.tensor(kmeans.labels_)
    labels = labels.reshape(256,20,1).float()
    hist = []
    for t_idx in range(labels.shape[1]):
        h_idx,_ = torch.histogram(labels[:,t_idx],bins=k,range=(0,k))
        hist.append(h_idx[None,:])
    
    hist = torch.cat(hist, dim=0).numpy()
    print(hist.shape)
    from matplotlib import pyplot as plt 
    figure = plt.figure()
    hist_plot = plt.imshow(hist, cmap='plasma', interpolation='nearest') 
    plt.colorbar(hist_plot) 
    plt.savefig(os.path.join(run.dir, f'hist_plot.png')) 


    # test 
    test_labels = kmeans.predict(z_test.numpy())
    test_labels = torch.tensor(test_labels)
    test_labels = test_labels.reshape(128,40,1).float()
    hist = []
    for t_idx in range(test_labels.shape[1]):
        h_idx,_ = torch.histogram(test_labels[:,t_idx],bins=k,range=(0,k))
        hist.append(h_idx[None,:])
    hist = torch.cat(hist, dim=0).numpy()
    print(hist.shape)
    figure = plt.figure()
    hist_plot = plt.imshow(hist, cmap='plasma', interpolation='nearest') 
    plt.colorbar(hist_plot) 
    plt.savefig(os.path.join(run.dir, f'test_hist_plot.png')) 

    origin = torch.tensor(kmeans.cluster_centers_).cuda()
    # origin = z_train[:4].cuda()
    coords = grid_tr[0:origin.shape[0], :, :, :, 0].contiguous().cuda()
    # print(coords.shape)
    with torch.no_grad():
        # origin = inr.latent_init.detach()
        origin_image = inr.modulated_forward(coords, origin).detach().cpu().numpy()
