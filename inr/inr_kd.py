...
from coral.metalearning import outer_step_metagrid, outer_step, outer_step_metagrid_twoview, outer_step_kd
...
@hydra.main(config_path="config/", config_name="siren_kd.yaml")
def main(cfg: DictConfig) -> None:
    ...
    kd_num_coords = cfg.optim.kd_num_coords
    ...
    teacher_checkpoint = torch.load(cfg.wandb.teacher_checkpoint_path, map_location='cpu')
    teacher_cfg = teacher_checkpoint['cfg']
    teacher_latent_dim = teacher_cfg.inr.latent_dim
    teacher_inr = create_inr_instance(
        teacher_cfg, input_dim=input_dim, output_dim=output_dim, device="cuda:0"
    )
    teacher_inr.load_state_dict(teacher_checkpoint['inr'])
    teacher_epoch_start = teacher_checkpoint['epoch']
    teacher_alpha = teacher_checkpoint['alpha'].detach().cuda()
    best_loss = teacher_checkpoint['loss']
    # cfg = checkpoint['cfg']

    teacher_inr.requires_grad = False
    # teacher_alpha.requires_grad = False
    for param in teacher_inr.parameters():
        param.requires_grad = False

    print(f"teacher {cfg.wandb.teacher_checkpoint_path}, epoch {teacher_epoch_start}, alpah {teacher_alpha.item()}, best_loss {best_loss}")
    ...
    for step in range(epoch_start, epochs):
        ...
        for substep, (images, modulations, coords, idx) in enumerate(train_loader):
            ...
            outputs = outer_step_kd(
                ...
                teacher_modulations=torch.zeros((n_samples, teacher_latent_dim), device=input_modulations.device),
                kd_num_coords=kd_num_coords,
            )
            ...
            teacher_fit_train_mse += outputs["teacher_loss"].item() * n_samples
            # mlp regression
            if use_rel_loss:
        ...
        teacher_fit_train_mse = teacher_fit_train_mse / ntrain
        if two_view:
        ...
        if True in (step_show, step_show_last):
            print(f'{step} alpha {alpha.item()}, teacher train loss {teacher_fit_train_mse}, train loss {train_loss}, test loss {test_loss}, test loss on train coords {test_loss_train_coords}, loss_mod_con: {loss_mod_con}')
            wandb.log(
                {
