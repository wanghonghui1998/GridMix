    ...
    for step in range(epoch_start, epochs):
        ...
        for substep, (images, modulations, coords, idx) in enumerate(train_loader):
            ...
            if w_mod_con > 0:
                ...
                loss_mod_con += outputs["loss_mod_con"].cpu().detach().item() * n_samples
            else:
        ...
        if True in (step_show, step_show_last):
            print(f'{step} alpha {alpha.item():.4e}, alpha_grid {alpha_grid.item():.4e}, train loss {train_loss:.4e}, test loss {test_loss:.4e},  test loss on diff test coords {diff_test_loss_train_coords:.4e}, test loss on train coords {test_loss_train_coords:.4e}, train loss_mod_con: {loss_mod_con:.4e}, test loss_code_con: {test_loss_code_con:.4e}, test loss_grid_con: {test_loss_grid_con:.4e}')
            wandb.log(
                {
