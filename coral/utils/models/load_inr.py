...
from coral.siren import ModulatedSirenGridMix, ModulatedSiren, ModulatedSirenGridMixSW
...
def create_inr_instance(cfg, input_dim=1, output_dim=1, device="cuda"):
    ...
    elif cfg.inr.model_type == "siren_GridMix_sw":
        print(cfg.inr.siren_init)
        inr = ModulatedSirenGridMixSW(
            dim_in=3,
            dim_hidden=cfg.inr.hidden_dim,
            dim_out=output_dim,
            num_layers=cfg.inr.depth,
...
def load_inr_model_inference_time(
    run_dir, run_name, data_to_encode, input_dim=1, output_dim=1, device="cuda", grid_size=8, grid_base=32
):  
    inr_train = torch.load(run_dir / f"{run_name}.pt")
    ...
    with open_dict(cfg):
        cfg.inr.grid_size = grid_size
        cfg.inr.grid_size_2 = grid_size
        cfg.inr.grid_base = grid_base
    print(f'{run_name}, epoch {inr_train["epoch"]}, alpha1 {alpha.item()}')
    inr = create_inr_instance(cfg, input_dim, output_dim, device)
    # inr.load_state_dict(inr_state_dict)
    # new_state_dict = {}
