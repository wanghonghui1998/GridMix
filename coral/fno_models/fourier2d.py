...
class FNO2dAR(nn.Module):
    def __init__(self, modes1, modes2,
        ...
        self.model = FNO2d(modes1, modes2, width=width, layers=layers, fc_dim=fc_dim, in_dim=in_dim, out_dim=out_dim, act=act, pad_ratio=pad_ratio)
    ...
    def forward(self, x):
        ...
        input_frame = x[:,:,:,0]
        input_concat = x[:,:,:,0,1:]
        for t_id in range(x.shape[3]-1):
            # print(input_frame.shape)
            out = self.model(input_frame)
            pred.append(out.unsqueeze(3))
            input_frame = torch.cat([out, input_concat], dim=-1)
        return torch.cat(pred, dim=3)
