...
class MLP4(nn.Module):
    def __init__(self, code_size, output_size, hidden_size, depth=1, nl="swish"):
        super().__init__()
...
class FNOMLP(nn.Module):
    def __init__(self, input_dim, modes1, modes2, width, num_channels):
        ...
        self.input_code = MLP4(input_dim, self.width, 512, depth=2, nl="swish")
        self.output_code = MLP4(128, input_dim, 512, depth=2, nl="swish")
        # self.grid = None 
        self.grid_per_bs = None

    def forward(self, code, x):
        input_code = self.input_code(code)  # batch_size, width
        grid = self.get_grid(x.shape, x.device)
        # print(x.shape, grid.shape)
        x = torch.cat((x, grid), dim=-1)
        x = self.fc0(x)
        ...
        # insert code in the middle 
        x = x + input_code.unsqueeze(-1).unsqueeze(-1)
        ...
        x_avg = x.mean(1).mean(1)
        output_code = self.output_code(x_avg)
        x = self.fc2(x)
        return output_code, x
...
class DerivativeCodeGridConcat(nn.Module):
    def __init__(self, input_dim, modes1, modes2, width, grid_channel=6, grid_size=64, **kwargs):
        super().__init__()
        self.grid_channel = grid_channel
        self.grid_size = grid_size
        self.input_dim = input_dim
        self.net_grid = FNO2d(modes1, modes2, width, grid_channel+input_dim)
        self.grad_code_linear = nn.Linear(input_dim, input_dim)
        # self.net_code = MLP2(input_dim, hidden_c, depth=depth, nl="swish")

    def forward(self, t, u):
        B,C=u.shape
        code = u[:, :self.input_dim]
        grid = u[:, self.input_dim:].reshape(u.shape[0], self.grid_channel, self.grid_size, self.grid_size).permute(0,2,3,1)
        code_grid = torch.cat([code.reshape(B,1,1,self.input_dim).repeat(1,self.grid_size, self.grid_size,1), grid], dim=-1)
        # u_grid = u.reshape(u.shape[0], self.grid_channel, self.grid_size, self.grid_size).permute(0,2,3,1)
        grad = self.net_grid(code_grid)
        grad_code = self.grad_code_linear(grad[...,:self.input_dim].reshape(B,-1,self.input_dim).mean(1))
        grad_grid = grad[...,self.input_dim:].permute(0,3,1,2).reshape(B, -1)

        # grad_code = self.net_code(code)

        return torch.cat([grad_code, grad_grid], dim=-1)

class DerivativeCodeGridGlobal(nn.Module):
    def __init__(self, input_dim, modes1, modes2, width, grid_channel=6, grid_size=64, **kwargs):
        super().__init__()
        self.grid_channel = grid_channel
        self.grid_size = grid_size
        self.input_dim = input_dim
        self.net = FNOMLP(input_dim, modes1, modes2, width, grid_channel)
    ...
    def forward(self, t, u):
        ...
        grad_code, grad_grid = self.net(code, grid)
        
        return torch.cat([grad_code, grad_grid.permute(0,3,1,2).reshape(B, -1)], dim=-1)

class DerivativeCodeGrid(nn.Module):
    def __init__(self, input_dim, hidden_c, depth, modes1, modes2, width, grid_channel=6, grid_size=64, **kwargs):
        super().__init__()
        self.grid_channel = grid_channel
        self.grid_size = grid_size
        self.input_dim = input_dim
        self.net_grid = FNO2d(modes1, modes2, width, grid_channel)
        self.net_code = MLP2(input_dim, hidden_c, depth=depth, nl="swish")
